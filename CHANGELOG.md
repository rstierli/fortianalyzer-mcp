...
- **Reports: custom `"start|end"` time ranges are now forwarded to FAZ.** `run_report`/`run_and_wait_report` sent `time-period: "other"` with no window, so FAZ ran the report over a default period instead of the requested one. The window is parsed and sent as `period-start`/`period-end`. Unknown presets (e.g. `"14-day"`) now raise a validation error instead of silently becoming `last-7-days`, and the report device filter uses `build_device_filter` instead of hardcoding `devid`.
