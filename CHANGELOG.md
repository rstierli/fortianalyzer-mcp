...
- **Reports: custom `"start|end"` time ranges now actually take effect.** `run_report`/`run_and_wait_report` sent `time-period: "other"` with no window, so FAZ ran the report over the layout's default period instead of the requested one (verified live on 7.6.7 and 8.0.0). The run endpoint has no window fields at all, so the window is configured on the layout's schedule config object (`update /config/adom/{adom}/sql-report/schedule/{layout_id}`: numeric `time-period` 16 = "other" + `period-start`/`period-end` in timedate format), verified by read-back (an unaccepted window aborts the run with an error instead of silently covering the wrong period), and the previous schedule period is restored after the run is submitted. Unknown presets (e.g. `"14-day"`) now raise a validation error instead of silently becoming `last-7-days`, and the report device filter uses `build_device_filter` instead of hardcoding `devid`.
